...
class PagesStore {
    ...
    @observable
    addRecent = (card: JSX.Element) => {
        this.recentCards.unshift(card);
    };

    @observable
    removeRecent = (identifier: string) => {
        this.recentCards = this.recentCards.filter((value: JSX.Element) => {
            if (identifier !== value.props.identifier) return value;
        });
    };

    @observable
    addBookmark = (card: JSX.Element) => {
    ...
    @observable
    removeBookmark = (tags: string[], category: string, identifier: string) => {
        this.bookmarkedCards.categories[category].filter(
            (value: JSX.Element) => {
                if (value.props.identifier !== identifier) return value;
            }
        );
    ...
    showPage = (index: number) => {
        switch (index) {
            case 1: {
                const categories: string[] = Object.getOwnPropertyNames(
                    this.bookmarkedCards.categories
                );
                categories.shift();
                categories.sort();
